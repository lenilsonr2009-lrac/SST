...
function filterNRs() {
  const searchInput = document.getElementById('nrSearch');
  const nrCounter = document.getElementById('nr-counter');
  ...
  if (nrCounter) {
    nrCounter.textContent = visibleCount > 0 ? `Exibindo ${visibleCount} normas.` : 'Nenhuma norma encontrada. Ajuste o filtro ou a busca.';
  }

  document.querySelectorAll('.nr-gerais, .nr-especiais, .nr-setoriais, .nr-revogadas').forEach(category => {
    const visibleCards = Array.from(category.querySelectorAll('.nrcard'))
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (newsModalBtn && newsModal) {
    newsModalBtn.addEventListener('click', () => {
      ...
      newsModalBtn.setAttribute('aria-expanded', 'true');
      newsModal.setAttribute('aria-hidden', 'false');
      newsModal.focus();
    });
    newsModalClose.addEventListener('click', () => {
      newsModal.classList.remove('active');
      newsModalBtn.setAttribute('aria-expanded', 'false');
      ...
      return `
        <div class="avoid-card ${favorites.includes(video.id) ? 'is-favorite' : ''} ${watched.includes(video.id) ? 'watched' : ''}" onclick="openVideo(${video.id})">
          ...
          <div class="avoid-card-thumbnail">
            <img src="${thumbUrl}" alt="${video.title}" class="card-img" loading="lazy" decoding="async">
  ...
  avoidBtn.addEventListener('click', () => {
    ...
    avoidBtn.setAttribute('aria-expanded', 'true');
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      if (playerModal.classList.contains('active')) {
        ...
        newsModal.classList.remove('active');
        newsModal.setAttribute('aria-hidden', 'true');
        if (newsModalBtn) newsModalBtn.setAttribute('aria-expanded', 'false');
      }
    }
  });
});